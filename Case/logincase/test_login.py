...
class TestLogin:
    ...
    def test_P1_login(self, start_module, item):
        ...
        ret = BaseRequest(url=item['url'], headers=start_module[0], method=item['method'],
                                                 data=json.dumps(item['parm'])).get_json()


        try:
            logger.info("期望值：{'code': 0, 'message':".format(str(item['excepted'])))
            logger.info("实际值：{'code': 0, 'message':".format(str(ret['meta']['status'])))
